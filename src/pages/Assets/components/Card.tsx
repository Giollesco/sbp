...
const Card = ({ asset, onClick }: Props) => {
  ...
  return (
    ...
    >
      <Group w="100%" h={200} bg="red.100" pos="relative">
        <Box w="100%" h={200} pos="absolute" top={0} left={0} bg={theme.colorScheme === "dark" ? "gray.8" : "gray.2"} />
        <Image
          fit="cover"
          width="100%"
          height={200}
      ...
      <Flex direction="row" align="center" justify="space-between" px="md" h={60}>
        ...
        <Flex direction="row" align="center">
          <ActionIcon variant="transparent" mr="md" color="red" onClick={removeAsset}><IconTrash size="1rem" /></ActionIcon>
          <ActionIcon variant="light" color="green" onClick={() => onClick(asset)}><IconEdit size="1rem" /></ActionIcon>
        </Flex>
      </Flex>
    </UnstyledButton>
  );
